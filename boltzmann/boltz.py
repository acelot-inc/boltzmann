...
def score_vina(pdb_file):
    ...
    subprocess.run(['mk_prepare_ligand.py',   '-i', ligand,  '-o', ligand + '.pdbqt'],        check=True)
    subprocess.run(['mk_prepare_receptor.py', '-i', protein, '-o', protein, '--write_pdbqt'], check=True)
